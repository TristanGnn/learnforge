  ...
  return (
    <div onClick={onFlip} style={{ perspective: 1200, cursor: "pointer", margin: "8px 0" }}>
      <div style={{
        ...
        {/* Front — en flux normal, définit la hauteur du container */}
        <div style={{
          backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden",
          background: "#12121e", borderRadius: 18, border: "2px solid #7c3aed33",
          ...
          {slide.frontImage ? (
            <img src={slide.frontImage} alt="" style={{ width: "100%", height: "auto", borderRadius: 12, display: "block" }} onError={e => { e.target.style.display = "none"; }} />
          ) : (
        ...
        <div style={{
          ...
          background: "#12121e", borderRadius: 18, border: "2px solid #3b82f633",
          display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, gap: 16,
          overflow: "auto",
        }}>
          ...
          {slide.backImage && (
            <img src={slide.backImage} alt="" style={{ width: "100%", height: "auto", borderRadius: 12, display: "block" }} onError={e => { e.target.style.display = "none"; }} />
          )}
...
function PreviewSlide({ slide, onAnswer, answered, flippedCards, onFlipCard }) {
  ...
  if (slide.type === "image") {
    return (
      <div style={S.pCard}>
        <h2 style={S.pTitle}>{slide.title}</h2>
        {slide.imageUrl ? (
          <img src={slide.imageUrl} alt={slide.title} style={{ width: "100%", height: "auto", borderRadius: 12, display: "block", margin: "16px 0" }} onError={e => { e.target.style.display = "none"; }} />
        ) : (
  ...
  if (slide.type === "quiz") {
    return (
      <div style={S.pCard}>
        <span style={{ fontSize: 11, fontWeight: 700, color: "#7c3aed", letterSpacing: 2, textTransform: "uppercase" }}>Quiz</span>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: "#f1f5f9", margin: "10px 0 8px" }}>{slide.title}</h2>
        <p style={{ fontSize: 15, color: "#94a3b8", marginBottom: 24, lineHeight: 1.65 }}>{slide.question}</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {(slide.options || []).map((opt, i) => {
            const isCorrect = i === slide.correct;
            const wasSelected = answered === i;
            const show = answered !== undefined;
            let bg = "transparent", bdr = "1.5px solid #2a2a3e", clr = "#cbd5e1";
            if (show && isCorrect) { bg = "#22c55e12"; bdr = "1.5px solid #22c55e55"; clr = "#4ade80"; }
            else if (show && wasSelected && !isCorrect) { bg = "#ef444412"; bdr = "1.5px solid #ef444455"; clr = "#f87171"; }
            return (
              <button key={i} onClick={() => !show && onAnswer(i)}
                style={{ background: bg, border: bdr, color: clr, padding: "13px 18px", borderRadius: 12, cursor: show ? "default" : "pointer", textAlign: "left", fontSize: 14, transition: "all 0.18s", display: "flex", alignItems: "center", gap: 12, fontFamily: "'DM Sans', sans-serif", fontWeight: 500 }}
                onMouseEnter={e => { if (!show) { e.currentTarget.style.background = "#7c3aed18"; e.currentTarget.style.borderColor = "#7c3aed66"; e.currentTarget.style.color = "#e2e8f0"; } }}
                onMouseLeave={e => { if (!show) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "#2a2a3e"; e.currentTarget.style.color = "#cbd5e1"; } }}
              >
                <span style={{ width: 26, height: 26, borderRadius: 8, border: `1.5px solid ${show && isCorrect ? "#22c55e55" : show && wasSelected && !isCorrect ? "#ef444455" : "#2a2a3e"}`, background: show && isCorrect ? "#22c55e22" : show && wasSelected && !isCorrect ? "#ef444422" : "#16161e", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0, color: show && isCorrect ? "#4ade80" : show && wasSelected && !isCorrect ? "#f87171" : "#475569" }}>
                  {show && isCorrect ? "✓" : show && wasSelected && !isCorrect ? "✗" : String.fromCharCode(65 + i)}
                </span>
                {opt}
              </button>
            );
        ...
        {answered !== undefined && (
          <div style={{ marginTop: 16, padding: "11px 16px", borderRadius: 10, background: answered === slide.correct ? "#22c55e0e" : "#ef44440e", border: `1px solid ${answered === slide.correct ? "#22c55e28" : "#ef444428"}`, color: answered === slide.correct ? "#4ade80" : "#f87171", fontSize: 13, fontWeight: 500, display: "flex", alignItems: "center", gap: 8 }}>
  ...
  if (slide.type === "separator") {
    return (
      <div style={{ textAlign: "center", padding: "44px 40px" }}>
        <h1 style={{ fontSize: 26, fontWeight: 700, color: "#f1f5f9", margin: "0 0 10px", lineHeight: 1.3 }}>{slide.title}</h1>
        {slide.content && <p style={{ color: "#64748b", fontSize: 15, margin: "0 0 20px" }}>{slide.content}</p>}
        <div style={{ width: 36, height: 3, background: "linear-gradient(90deg, #7c3aed, #6366f1)", borderRadius: 4, margin: "0 auto" }} />
      </div>
    );
  }
  return null;
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#0c0c14", color: "#e2e8f0", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        {/* ─── PREVIEW ──────────────────────────────────────────── */}
        {mode === "preview" && (
          <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "#0a0a12" }}>
            {/* Top: module tabs + progress */}
            <div style={{ background: "#0e0e1a", borderBottom: "1px solid #1a1a28", padding: "14px 28px", flexShrink: 0 }}>
              <div style={{ display: "flex", gap: 6, marginBottom: 14, overflowX: "auto", paddingBottom: 2 }}>
                {course.modules.map((mod, mi) => (
                  <button key={mod.id} onClick={() => setPreviewModuleIdx(mi)} style={{
                    padding: "7px 18px", borderRadius: 10, border: "1.5px solid", cursor: "pointer", fontSize: 12, fontWeight: 600,
                    background: previewModuleIdx === mi ? "#7c3aed" : "transparent",
                    borderColor: previewModuleIdx === mi ? "#7c3aed" : "#2a2a3e",
                    color: previewModuleIdx === mi ? "#fff" : "#64748b",
                    whiteSpace: "nowrap", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s",
                  }}
                    onMouseEnter={e => { if (mi !== previewModuleIdx) { e.currentTarget.style.borderColor = "#7c3aed88"; e.currentTarget.style.color = "#94a3b8"; } }}
                    onMouseLeave={e => { if (mi !== previewModuleIdx) { e.currentTarget.style.borderColor = "#2a2a3e"; e.currentTarget.style.color = "#64748b"; } }}
                  >
                    {(scrollPercents[mi] || 0) >= 0.9 && mi !== previewModuleIdx && <span style={{ marginRight: 5, color: "#22c55e" }}>✓</span>}
                    {mod.title}
                  </button>
                ))}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 11, color: "#475569", fontWeight: 600, letterSpacing: 0.5, minWidth: 80, textTransform: "uppercase" }}>Progression</span>
                <div style={{ flex: 1, height: 5, background: "#1a1a28", borderRadius: 6, overflow: "hidden" }}>
                  <div style={{ width: `${progress}%`, height: "100%", background: progress >= 100 ? "linear-gradient(90deg, #22c55e, #10b981)" : "linear-gradient(90deg, #7c3aed, #6366f1)", borderRadius: 6, transition: "width 0.6s cubic-bezier(0.4,0,0.2,1)" }} />
                </div>
                <span style={{ fontSize: 12, color: progress >= 100 ? "#22c55e" : "#7c3aed", fontWeight: 700, minWidth: 36, textAlign: "right" }}>{progress}%</span>
              </div>
            </div>

            {/* Scrollable: ALL slides of current module on same page */}
            <div ref={scrollRef} onScroll={handleScroll} style={{ flex: 1, overflow: "auto", padding: "40px 24px 0" }}>
              <div style={{ maxWidth: 660, margin: "0 auto" }}>
                <div style={{ textAlign: "center", marginBottom: 48 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#475569", letterSpacing: 2, textTransform: "uppercase" }}>Module {previewModuleIdx + 1} / {course.modules.length}</span>
                  <h1 style={{ fontSize: 28, fontWeight: 700, color: "#f1f5f9", margin: "10px 0 0", lineHeight: 1.25 }}>{previewModule.title}</h1>
                  <div style={{ width: 40, height: 3, background: "linear-gradient(90deg, #7c3aed, #6366f1)", borderRadius: 4, margin: "18px auto 0" }} />
                </div>

                {previewModule.slides.map((slide) => (
                  <div key={slide.id} style={{ marginBottom: 24 }}>
                    <PreviewSlide
                      slide={slide}
            ...
            <div style={{ background: "#0e0e1a", borderTop: "1px solid #1a1a28", padding: "14px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
              <button onClick={() => setPreviewModuleIdx(Math.max(0, previewModuleIdx - 1))} disabled={previewModuleIdx === 0} style={{ ...S.btnSec, opacity: previewModuleIdx === 0 ? 0.25 : 1, cursor: previewModuleIdx === 0 ? "default" : "pointer", background: "transparent", borderColor: "#2a2a3e" }}>
                ← Précédent
              </button>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {course.modules.map((_, i) => (
                  <div key={i} style={{ width: i === previewModuleIdx ? 20 : 8, height: 8, borderRadius: 6, background: i === previewModuleIdx ? "#7c3aed" : i < previewModuleIdx ? "#7c3aed44" : "#2a2a3e", transition: "all 0.3s", cursor: "pointer" }} onClick={() => setPreviewModuleIdx(i)} />
                ))}
              </div>
              {previewModuleIdx < course.modules.length - 1 ? (
                <button onClick={() => setPreviewModuleIdx(previewModuleIdx + 1)} style={S.btnPri}>Suivant →</button>
              ) : (
...
const S = {
  ...
  pCard: { background: "#12121e", borderRadius: 16, padding: 28, border: "1px solid #1e1e2e" },
  pTitle: { fontSize: 20, fontWeight: 700, color: "#f1f5f9", marginBottom: 14 },
  pContent: { color: "#94a3b8", fontSize: 15, lineHeight: 1.8 },
};
